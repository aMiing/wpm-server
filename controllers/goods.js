...
const tableName = ctx => vertifyToken(ctx) + '_goods';
...
const fn_getList = async ctx => {
  ...
  const total_sql = `SELECT count(*) FROM ${tableName(ctx)} WHERE deleted=1` + type_sql; //返回总条数
  const sql =
    `SELECT * FROM ${tableName(ctx)} WHERE deleted=1 ${type_sql} ORDER BY createTime DESC limit ${
      pageNo * pageSize
    },` + pageSize;
  try {
...
const updateGoods = async (ctx, next) => {
  ...
  const { name, price, author, stock, type, online, uuid } = row;
  try {
    ...
    data = await ControlAPI_obj_async(`SELECT * FROM ${tableName(ctx)} WHERE uuid=?`, uuid);
    ctx.response.body = {
      code: 200,
      msg: '更新商品信息成功！',
      data,
...
const fn_resetStock = async (ctx, next) => {
  ...
  try {
    Object.keys(body).forEach(async e => {
      const ele = body[e];
      const sql = `UPDATE ${tableName(ctx)} SET stock=? WHERE uuid=?`;
