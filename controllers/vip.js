...
const tableName = ctx => vertifyToken(ctx) + '_vip';

// 获取订单列表
const fn_getList = async (ctx, next) => {
  ctx.response.head = 'text/plain; charset=UTF-8';
  const sql = `SELECT * FROM ${tableName(ctx)} WHERE deleted=1 ORDER BY createTime desc`;
  try {
...
const create = async (ctx, next) => {
  ...
  try {
    ...
    const data = await ControlAPI_obj_async(
      `SELECT * FROM ${tableName(ctx)} WHERE uuid=?`,
      row.uuid
    );
    ctx.response.body = {
      code: 200,
...
const deleteItem = async (ctx, next) => {
  ...
  const sql = `UPDATE ${tableName(ctx)} SET deleted=2 WHERE uuid=?`;
  try {
...
const getDataPreview = async (ctx, next) => {
  const row = ctx.request.body;
  const buffer = temp =>
    `SELECT SUM(realPayPrice) as volume,COUNT(uuid) as orderCount FROM ${tableName(
      ctx
    )} WHERE createTime >=FROM_UNIXTIME(${temp[0] / 1000}) AND createTime <= FROM_UNIXTIME(${
      temp[1] / 1000
    })`;
  const sqlArr = [];
...
const getviptatistics = async (ctx, next) => {
  ...
  const sql = `SELECT DATE_FORMAT(createTime, '%Y/%m/%d') as date,COUNT(uuid) as orderCount,SUM(realPayPrice) as volume FROM ${tableName(
    ctx
  )} WHERE createTime >=FROM_UNIXTIME(${range[0] / 1000}) AND createTime <= FROM_UNIXTIME(${
    range[1] / 1000
  }) AND deleted = 1 GROUP BY DATE_FORMAT(createTime, '%Y/%m/%d')`;
  try {
    const data = await ControlAPI_obj_async(sql);
    ctx.response.body = {
      code: 200,
